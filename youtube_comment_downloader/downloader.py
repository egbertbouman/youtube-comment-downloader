...
Extended by DrDayoX (https://github.com/drdayoX)
"""
...
def ajax_request(session, endpoint, ytcfg, retries=9, sleep=4):
    ...
    for _ in range(retries):
        ...
        if response.status_code in [403, 413]: return {}
    ...
    while stack:
        ...
        if isinstance(current_item, dict):
            for key, value in current_item.items():
                if key == search_key: yield value
                else: stack.append(value)
        elif isinstance(current_item, list):
            for value in current_item: stack.append(value)
